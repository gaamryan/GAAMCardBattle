...
3. **Voice** — announcer via `speechSynthesis`: `Voice.say(text)` (hooked into `bannerEpic` and TURN `banner`s), style from `CFG.voice.styles[CFG.voice.style]` (pitch/rate/voiceHint regex + `excite` flag: adds shouted interjections on SUPER/VICTORY/FINAL/SNAP/REVIVE lines), `_pickVoice` ranks the device's voices (natural/neural/Google first, then hint match, then en-US), local mute in `gaam_voice_off`, 🎙 top-bar pill, admin style select in the Music tab (serialized as `{enabled,style}` in Admin.store; travels in `netCfg` and the published config). Default style: `maxhype`. **Voice pack:** `Voice.preloadPack()` probes `assets/voice/<key>.mp3` for the keys in `PACK_KEYS` at boot; `Voice.say` first tries `playPack` (banner text → key via the `PACK_MAP` regex table — e.g. `TURN 3` → `round_3`, `…SUPER ATTACK` → `super_attack`) and only falls back to speech synthesis when no file loaded. Keys may ship a `<key>_explicit.mp3` variant, preferred when `CFG.voice.explicit` (admin toggle, default on) is set. Recording script: `docs/VOICE_PACK.md`.
...
6. **Game logic registries** — `ABILITIES.onReveal` / `ABILITIES.ongoing`, `LOCATION_EFFECTS` (`perCard` live bonus + `onCardRevealed` hook), `teamOf()`, `synergyActive()`. Location effects include `masterBonus` (cost-5+ +2), `loneBuilder` (+2 while your only card there), `underdog` (+2 to the side losing the location — computed from raw power+permBuff sums, NOT cardPower, to avoid recursion) and `chaosBless` (reveal-time −2..+3 via ctx.rng — shared-seeded, MP-safe). **Featured stage:** `featuredId()` (date-seeded pick; guests use `G._featuredOverride` from the host's netCfg, cleared in restoreCfg) is forced into the location deal; `cardPower` doubles that stage's `perCard` and forgeFirst doubles its buff via `isFeatured(li)`. **`affect(ctx, target, delta, staggerMs)` is the one true way an ability changes power**: it mutates `permBuff` synchronously (determinism) and schedules the visuals (projectile/impact/float) after `staggerMs`. It also increments `G._combo` for the combo counter.
